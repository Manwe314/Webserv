...
{
	/*
    	For the Server constuctor,
		we only need the host port pair and name.
		Hence, we use a map with string and t_host_port key value pair.
	*/
    std::map<std::string, t_host_port> servers;
	/*
    	Config member function that returns all successfully configured servers name
		and t_host_pair.        
	*/
    servers = conf.getAllPairs();

    //We need to iterate over every element of this map and for each ":"
    for (std::map<std::string, t_host_port>::iterator it = servers.begin(); it != servers.end(); it++)
    {
		/*
        	Call a server constuctor with t_host_port and name
			(fd set at -1, fd will be set by the setup() function of the server itself).
		*/
